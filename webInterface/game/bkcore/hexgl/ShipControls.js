...
bkcore.hexgl.ShipControls = function(ctx)
{
    ...
    conn.onclose = function(e) {
 	...
 	conn.onmessage = function(e) {
		...
		self.key.forward = partsOfStr[1] > -3;
		// self.key.ltrigger = partsOfStr[1] > 0;
		// self.key.rtrigger = partsOfStr[1] > 0;
		self.key.left = partsOfStr[0] < 5;
		self.key.right = partsOfStr[0] > -5;
...
bkcore.hexgl.ShipControls.prototype.update = function(dt)
{
	...
	if(this.active)
	{
		...
		else
		{
			if(this.key.left)
			{
				angularAmount += (this.angularSpeed * dt)/2;
